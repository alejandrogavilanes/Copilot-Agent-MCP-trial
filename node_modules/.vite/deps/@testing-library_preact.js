  ...
  "node_modules/deep-equal/index.js"(exports, module2) {
    ...
    function objEquiv(a, b2, opts, channel) {
      ...
      if (aIsArrayBuffer || bIsArrayBuffer) {
        ...
        return typeof Uint8Array === "function" && internalDeepEqual(new Uint8Array(a), new Uint8Array(b2), opts, channel);
      }
    ...
    module2.exports = function deepEqual(a, b2, opts) {
...
function u(u2) {
  ...
  if (r(v)) return v.then(y2, function(t) {
    throw y2(), t;
  });
  if (y2(), l2) throw l2;
...
Object.keys(fireEvent).forEach((key) => {
  fireEvent2[key] = (elem, init) => {
    const eventName = `on${key.toLowerCase()}`;
    const isInElem = eventName in elem;
    const keyFiltered = !isCompat ? key : renameEventCompat(key);
    return isInElem ? fireEvent[keyFiltered](elem, init) : fireEvent(elem, createEvent(keyFiltered[0].toUpperCase() + keyFiltered.slice(1), elem, init));
  };
});
...
function render(ui, {
  ...
  const wrapUiIfNeeded = (innerElement) => WrapperComponent ? _(WrapperComponent, null, innerElement) : innerElement;
  u(() => {
    if (hydrate) {
      G(wrapUiIfNeeded(ui), container);
    } else {
      E(wrapUiIfNeeded(ui), container);
    }
  });
  return {
    container,
...
function renderHook(renderCallback, options) {
  ...
  const {
    ...
    unmount
  } = render(_(TestComponent, {
    renderCallbackProps: initialProps
  }), {
    wrapper
  });
  function rerender(rerenderCallbackProps) {
    return baseRerender(_(TestComponent, {
...
react-is/cjs/react-is.development.js:
   ...
   *)
*/
//# sourceMappingURL=@testing-library_preact.js.map
